...
function JustDanceGame() {
    ...
    function verNotification() {
        var content_en = "New update " + version + " is out. Please update the Just Dance Now app on your device!";
        if (version === "0.1.5") {
            // old type
            var $newUpdateContent = $('<div class="new_update"><div class="message">' + content_en + '</div></div>');
            $newUpdateContent.appendTo(jLandingContent)
        }
    }
    ...
    function initLandingPage() {
        ...
        function debug(fpsCounter) {
            if (isDebug === true) {
                ...
                if (isDebug === true) {
                    jsAnimationTemplate(
                        console.log("debug: js animation"))
                }
            }
        }
        ...
        var roomGenerator = function () {
            ...
            if (ErrorPopUp === true) {
                ...
                jPopUp.addClass(hidden);
            }
        }

        function landingToast() {
            var isShow = false;
            var newFeatures = document.querySelector('new-features');
            ...
            var $toastTitle = $('.new-features .title');
            var $content = $('.new-features .content');

            $toastTitle.html(loc['313'][hash]);
            $content.html(loc['314'][hash]);

            var $rightToastClose = $('.new-features .button__close');

            if (newFeatures === true) {
                isShow = true;
                jFeatures.removeClass(hidden).addClass(visible);
                $rightToastClose.on('click', function () {
                    isShow = false;
                    jFeatures.removeClass(visible).addClass(hidden);
                });
            } else {
                isShow = false;
                jFeatures.remove();
        ...
        function popUp() {
            var $popUpClose = $('.close__button');
            if (ErrorPopUp === true) {
                jPopUp.removeClass(hidden).addClass(visible);
            } else {
        ...
        function adminMenu() {
            ...
            var $adminClose = $('#landing-admin-menu .menu__header .close');

            var $settingsTitle = $('.menu__header .title h1');
            var $langtitle = $('.language-changer .title');

            var $graphtitle = $('.graphics-changer .title');
            var $graphhig = $('.graphics-changer option[value="high"]');
            var $graphlow = $('.graphics-changer option[value="medium"]');
            var $graphmed = $('.graphics-changer option[value="low"]');

            var $fullstitle = $('.full-screen .title');
            var $fullscheck = $('.full-screen .check');

            var $bottomauthors = $('a.legacy-item.authors');
            var $bottomlicense = $('a.legacy-item.license');
            var $bottomfaq = $('a.legacy-item.faq');
        ...
        function landingModal() {
            ...
            $modalheadertitle = $('.modal__header .title'),
            $modalClose = $('#landing-modal .close'),
            $modalSteps = $(".modal_position"),
            $modalStep1 = $(".modal_position_1"),
            $modalStep1content = $(".modal_position_1 .modal_description"),
            $modalStep2 = $(".modal_position_2"),
            $modalStep2content = $(".modal_position_2 .modal_description"),
            $modalStep3 = $(".modal_position_3"),
            $modalStep3content = $(".modal_position_3 .modal_description");

            $modalOpen.html(loc['340'][hash]);
            $modalheadertitle.html(loc['331'][hash]);
            ...
            function modalVisible() {
                UI_Search_a01.play();
                UI_Snap_01.play();
                jHTML.removeClass("dialog").addClass("modalActive");
                jLandingModal.addClass(visible).css({
                    'display': 'block'
                });
                setTimeout(() => $(function () {
            ...
            function modalHidden() {
                ...
                jLandingModal.removeClass(visible).css({
                    'display': 'none'
                });
        ...
        function secondScreenPrepare() {
            var screenAppend = 0;
            var $screenStructure = $('<div id="second-screen" role="second-screen"><div class="title">LOC 344</div></div>');
            if (screenAppend === 0) {
                screenAppend = 1;
                $screenStructure.appendTo(jBody);
            }

            // screen
            var $allScreen = $('#second-screen');
            var $title = $allScreen.find('.title');
